...
def delete_meter(meter_id):
    ...
    try:
        ...
        # Check if meter is assigned to a unit
        assigned_unit = Unit.query.filter(
        ...
        if assigned_unit:
            return jsonify({
                "error": f"Cannot delete meter: assigned to Unit {assigned_unit.unit_number}. Unassign it first."
            }), 400
        ...
        db.session.rollback()
        return jsonify({"error": f"Failed to delete meter: {str(e)}"}), 500
